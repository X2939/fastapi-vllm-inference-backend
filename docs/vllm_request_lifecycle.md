...
> The local service uses FastAPI for `/chat` and `/chat/stream`, with vLLM providing the OpenAI-compatible model-serving layer. The request path covers parsing, chat-template rendering, `SamplingParams`, `AsyncLLM`, `EngineCore`, scheduling, KV-cache allocation, model execution, detokenization, and final JSON/SSE output. Benchmark scripts measure latency, P50/P95, throughput, tokens/s, and TTFT under controlled workloads.
